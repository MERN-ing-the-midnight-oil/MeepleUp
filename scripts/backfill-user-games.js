...
/**
 * Backfill User Games Script
 * Only backfills games that actual users have in their collections
...
async function updateGameInFirestore(gameId, bggData) {
  ...
  if (!doc.exists) {
    ...
    await gameRef.set({
      ...
      averageWeight: bggData.averageWeight || null,
      bggDataCached: true,
  ...
  try {
          ...
          gamesSnapshot.docs.forEach(doc => {
            ...
            if (gameId) {
              ...
              if (!userGameMap.has(gameIdStr)) {
                userGameMap.set(gameIdStr, { 
                  count: 0, 
                  users: new Set(),
                  name: gameData.title || gameData.name || 'Unknown'
                });
              }
    ...
    for (let i = 0; i < gameIdArray.length; i += batchSize) {
      ...
      gameDocs.forEach((doc, index) => {
        ...
        if (!doc.exists) {
          // Game not in Firestore - needs to be created
          ...
          const gameData = doc.data();
          const gameName = gameData.name || gameData.title || gameInfo.name;
      ...
      // Progress indicator
      if ((i + batchSize) % 500 === 0 || i + batchSize >= gameIdArray.length) {
        console.log(`   Checked ${Math.min(i + batchSize, gameIdArray.length)}/${gameIdArray.length} games...`);
      }
    }
    
    console.log(`\n✅ Complete games: ${completeCount}`);
    console.log(`⚠️  Incomplete games: ${incompleteGames.length}\n`);
    
    if (incompleteGames.length === 0) {
      console.log('🎉 All games in user collections are complete! No backfill needed.\n');
      return;
    }
    
    // Sort by number of affected users (prioritize games affecting more users)
    incompleteGames.sort((a, b) => b.affectedUsers - a.affectedUsers);
    ...
    for (let i = 0; i < incompleteGames.length; i++) {
      ...
      console.log(`   Affects ${game.affectedUsers} users, ${game.userCount} instances`);
